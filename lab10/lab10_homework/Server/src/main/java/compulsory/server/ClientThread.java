...
public class ClientThread extends Thread {
    ...
    public ClientThread(Socket socket, PersonRepository personRepository, MessageRepository messageRepository) {
        this.socket = socket;
        ...
        try {
            ...
            while (true) {
                ...
                String result = switch (parsedCommand[0]) {
                    ...
                    case "send" -> send(request.substring(5));
                    case "read" -> read();
    ...
    public String send(String message) {
        if (person != null) {
            messageRepository.save(new Message(person.getId(), message));
    ...
    /**
     * takes the messages from the database that are sent by the connected users friends
     */
    public String read() {
        ...
        for (Person friend : friends) {
            for (Message message : friend.getMessages())
                result.append(friend.getName()).append(": ").append(message.getText()).append("; ");
        }
        if (result.isEmpty())
    ...
    private String friend(String[] command) {
        if (person == null) return "You are not logged in!";
        StringBuilder result = new StringBuilder();
